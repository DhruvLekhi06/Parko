...
- 2026-09-17 (presentation polish): Explore rows show the locality after the distance (`locality(address)` = last comma segment, ellipsis on overflow) because 103 Bengaluru venues share a name (13 "Government Hospital"). BookSheet calls `onHeld` the moment the booking succeeds so the floor panel switches to the booking card behind the ticket (no more "Held by another driver" on your own slot). Activity session detail shows "Unused credit returned". Operator Drivers tile and Users tab count only accounts or guests with activity. In-garage route line no longer shows a drive time.
- 2026-09-17 (deploy day): Exit billing fixed after the deployed E2E run. Any stay bills at least 1 min (`server/records.js` elapsedMinutes ceils, matching the client feeFor) so the ₹50 first hour is always charged; `exitCharge` returns `unused` (booking fee beyond the parking fee) and `endSession` refunds it to the wallet as a `refund` transaction "Unused booking credit" (60-min ₹80 booking + 1 h parking = ₹50 net). ExitSheet/receipt rows: "Booking fee already paid", "Unused booking credit, back to wallet", "Covered by your booking fee". Operator Users tab hides device guests with no activity.
...
## Current state
- 2026-09-17 15:20: LIVE on Vercel (https://spot-lrzs9d37m-dhruvlekhi06s-projects.vercel.app, Supabase Postgres). Vercel Deployment Protection is still ON (non-logged-in visitors get a 302 to Vercel login): Dhruv must switch off Vercel Authentication under Project → Settings → Deployment Protection and use the production domain. Full E2E on the deployment passed for a fresh account (signup → FASTag → book 30 min → insufficient → top up ₹100 and book → ticket → +15 with confirm → I've parked → exit → receipt SP-2026-000002 → Activity → operator portal). Two billing bugs found there and fixed locally (see Decisions), verified on a local build; commit pending Dhruv's push. Bengaluru import into Supabase died on ETIMEDOUT at 1,271 venues; rerun in progress (242 places left). Supabase report views (report_users, report_transactions, report_bookings, report_parking) show the evaluator where data lands.
