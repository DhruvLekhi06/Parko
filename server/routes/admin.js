...
const KEY = process.env.ADMIN_KEY || 'spoton-admin';
const IST_DAY = `(date_trunc('day', now() at time zone 'Asia/Kolkata') at time zone 'Asia/Kolkata')`;

export const router = Router();

router.use((req, res, next) => {
  if (req.get('X-Admin-Key') !== KEY) throw new ApiError(401, 'ADMIN_KEY', 'Operator key required');
  next();
});
...
router.get('/venues', async (req, res) => {
  const rows = await many(`select v.id, v.name, v.city, v.type, v.rate,
  ...
  res.json({
    venues: rows.map((r) => ({
      id: r.id, name: r.name, city: r.city, type: r.type, rate: parseJson(r.rate, {}), total: r.total, free: r.free, occupied: r.occupied, reserved: r.reserved,
...
router.put('/venues/:id', async (req, res) => {
  const v = await one(`select id, rate from venues where id = $1`, [req.params.id]);
  ...
  await run(`update venues set rate = $1 where id = $2`, [JSON.stringify(rate), v.id]);
  res.json({ id: v.id, rate });
});
